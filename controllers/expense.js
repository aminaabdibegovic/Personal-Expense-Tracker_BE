...
const getYearsFromExpenseDate = async (id) => {
  try {
    const result = await Expense.findAll({
      attributes: [[Sequelize.fn('EXTRACT', Sequelize.literal('YEAR FROM expense_date')), 'year']],
      where: { user_id: id },
      group: [Sequelize.literal('EXTRACT(YEAR FROM expense_date)')],
      order: [Sequelize.literal('EXTRACT(YEAR FROM expense_date)')],
    });
...
const getSumThisMonth = async (id, month) => {
  try {
    const result = await Expense.findOne({
      attributes: [[Sequelize.fn('SUM', Sequelize.col('amount')), 'total_sum']],
      where: {
        user_id: id,
        [Sequelize.Op.and]: [
          Sequelize.where(
...
module.exports = {
  ...
  totalSumByCategory,
  totalSumByMonth,
  getYearsFromExpenseDate,
  getMonthsFromExpenseDate,
  getSumThisMonth,
};
